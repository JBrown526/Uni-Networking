...
public class RandomStoryGen {
    ...
    private static final String CHARACTER_DETAILS_FORMAT = "%s %s, a %s";
    private static final String TWO_ELEMENT_SPACED_FORMAT = "%s %s";
    private static final String FIVE_ELEMENT_SPACED_FORMAT = "%s %s %s %s %s";
    ...
    // arrays for names of people and places
    ...
    public String makeStory() {
        ...
        if (gender == CharacterGender.MALE) {
            ...
            characterSegment = String.format(CHARACTER_DETAILS_FORMAT, getRandom(maleNames), getRandom(surnames), getRandom(maleJobs));
        } else {
            // selects random details for a female character
            characterSegment = String.format(CHARACTER_DETAILS_FORMAT, getRandom(femaleNames), getRandom(surnames), getRandom(femaleJobs));
        }
        ...
        switch (quest) {
            case MULTI_MALE:
                ...
                questSegment = String.format(FIVE_ELEMENT_SPACED_FORMAT, questSegment, getRandom(multiMaleQuests), getRandom(maleTitles), getRandom(maleNames), genderedEnd);
                break;
            case MULTI_FEMALE:
                // selects random details for a quest with a female target character
                questSegment = String.format(FIVE_ELEMENT_SPACED_FORMAT, questSegment, getRandom(multiFemaleQuests), getRandom(femaleTitles), getRandom(femaleNames), genderedEnd);
                break;
            default:
                // selects random details for a quest with no target character
                questSegment = String.format(TWO_ELEMENT_SPACED_FORMAT, questSegment, getRandom(quests));
        }
    ...
    public String makeTragicBackstory() {
        ...
        characterSegment = String.format(TWO_ELEMENT_SPACED_FORMAT, characterSegment, getRandom(surnames));

        // populates random details for a villain
        if (villainGender == CharacterGender.MALE) {
            // random details for a male villain
            villainSegment = String.format(TWO_ELEMENT_SPACED_FORMAT, getRandom(maleTitles), getRandom(maleNames));
        } else {
            // random details for a female villain
            villainSegment = String.format(TWO_ELEMENT_SPACED_FORMAT, getRandom(femaleTitles), getRandom(femaleNames));
        }
